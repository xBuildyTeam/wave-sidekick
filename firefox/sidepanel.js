// Wave Sidekick — Side Panel Logic (Firefox MV2)
// Chat, memory, CoS actions, voice input (STT), text-to-speech (TTS), heartbeat
...
(async function init() {
  ...
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden && config.authToken) {
      api.runtime.sendMessage({ type: "START_HEARTBEAT" });
    } else {
      api.runtime.sendMessage({ type: "STOP_HEARTBEAT" });
    }
  });
})();
...
function initSpeechSynthesis() {
  if (!window.speechSynthesis) {
    console.warn("[Wave Sidekick FF] Speech Synthesis not available");
...
function speak(text) {
  ...
  let chunkIndex = 0;
  function speakChunk() {
    if (chunkIndex >= chunks.length) { stopSpeaking(); return; }
    ...
    utter.onend = () => { chunkIndex++; if (chunkIndex < chunks.length && isSpeaking) speakChunk(); else stopSpeaking(); };
...
speakBtn.addEventListener("click", () => { if (lastAssistantText) speak(lastAssistantText); });
...
function initSpeechRecognition() {
  ...
  if (!SpeechRecognition) {
    console.warn("[Wave Sidekick FF] Web Speech API not available in Firefox");
    micBtn.disabled = true;
    micBtn.title = "Voice input not available in Firefox (use Chrome for STT)";
    return;
  }
  ...
  recognition.onstart = () => {
    ...
    micIcon.style.display = "none"; micStopIcon.style.display = "block";
    voiceOverlay.style.display = "flex"; voiceHint.style.display = "block";
    voiceStatus.textContent = "Listening…"; voiceInterim.textContent = "";
  ...
  recognition.onerror = (event) => {
    if (event.error === "no-speech") voiceStatus.textContent = "No speech detected…";
    else if (event.error === "not-allowed") { stopRecording(); addMessage("assistant", "⚠️ Microphone access denied."); }
    else if (event.error === "network") stopRecording();
  ...
  recognition.onend = () => {
    ...
    micIcon.style.display = "block"; micStopIcon.style.display = "none";
    voiceOverlay.style.display = "none"; voiceHint.style.display = "none";
...
function resetSilenceTimer() {
  ...
  silenceTimer = setTimeout(() => {
    if (isRecording && recognition) { voiceStatus.textContent = "Stopping…"; try { recognition.stop(); } catch (e) {} }
  }, silenceTimeout);
}
...
function startRecording() {
  ...
  catch (e) { try { recognition.stop(); setTimeout(() => recognition.start(), 200); } catch (e2) {} }
}
...
function setConfig(updates) {
  return new Promise((resolve) => {
    api.storage.local.get(["waveConfig"], (result) => {
      ...
      api.storage.local.set({ waveConfig: { ...current, ...updates } }, resolve);
    });
  });
}
...
function showAuthGate() { authGate.style.display = "flex"; mainPanel.style.display = "none"; settingsPanel.style.display = "none"; }
function showMainPanel() { authGate.style.display = "none"; mainPanel.style.display = "flex"; settingsPanel.style.display = "none"; }
function showSettings() { authGate.style.display = "none"; mainPanel.style.display = "none"; settingsPanel.style.display = "flex"; loadSettings(); }
...
document.getElementById("connect-btn").addEventListener("click", async () => {
  ...
  if (!token) { errEl.textContent = "Please enter your Wave OS auth token."; errEl.style.display = "block"; return; }

  try {
    await setConfig({ authToken: token, workspaceId: wsId });
    errEl.style.display = "none";
    showMainPanel();
    addMessage("assistant", "Connected to Wave OS! I'm ready to help. Tap the speaker icon to hear my responses!");
    fetchCredits();
    startHeartbeat();
  } catch (e) {
...
document.getElementById("open-oswave").addEventListener("click", () => {
  api.tabs.create({ url: "https://oswave.io" });
});
...
document.getElementById("tts-test").addEventListener("click", () => {
  speak("Hey! I'm your Wave Assistant. This is a test of my text-to-speech voice.");
});
...
document.getElementById("context-btn").addEventListener("click", async () => {
  api.runtime.sendMessage({ type: "GET_PAGE_CONTEXT" }, (response) => {
    if (response && response.ok) { pageContext = response.context; showContextBanner(); }
  });
});
...
document.getElementById("context-clear").addEventListener("click", () => {
  pageContext = null; contextBanner.style.display = "none";
});
...
async function handleQuickAction(action) {
  if (action === "briefing") { addMessage("user", "Give me my morning briefing"); await callWaveFunction("waveChiefOfStaff", { action: "morningBriefing" }); }
  else if (action === "triage") { addMessage("user", "Run triage check"); await callWaveFunction("waveChiefOfStaff", { action: "triage", proactivity_level: "medium" }); }
  else if (action === "followups") { addMessage("user", "Show my follow-ups"); await callWaveFunction("waveChiefOfStaff", { action: "followUpScan" }); }
  else if (action === "memory") { addMessage("user", "Show my saved memories"); await callWaveFunction("waveChat", { action: "listMemory" }); }
}
...
async function sendMessage() {
  ...
  try {
    const dupRes = await api.runtime.sendMessage({
      type: "API_CALL",
      payload: {
        function: "waveChatSync",
  ...
  try {
    const storeRes = await api.runtime.sendMessage({
      type: "API_CALL",
      payload: {
        function: "waveChatSync",
        body: {
  ...
  await callWaveFunction("waveChat", { action: "chat", message: text, context: pageContext });
...
async function callWaveFunction(funcName, body) {
  if (isTyping) return;
  isTyping = true; typingIndicator.style.display = "flex";
  sendBtn.disabled = true; if (recognition) micBtn.disabled = true;

  try {
    const response = await api.runtime.sendMessage({ type: "API_CALL", payload: { function: funcName, body } });
    if (!response.ok) { addMessage("assistant", "⚠️ Error: " + response.error); return; }
    renderResponse(funcName, body.action, response.data);
  } catch (e) {
    addMessage("assistant", "⚠️ Connection error: " + e.message);
  } finally {
    isTyping = false; typingIndicator.style.display = "none";
    sendBtn.disabled = false; if (recognition) micBtn.disabled = false;
  }
}
...
function renderResponse(funcName, action, data) {
  ...
  if (funcName === "waveChiefOfStaff") { renderCoSResponse(action, data); return; }
  addMessage("assistant", JSON.stringify(data, null, 2));
}

function maybeSpeak(text) {
  const cleanText = text.replace(/\*\*(.+?)\*\*/g, "$1").replace(/\n{2,}/g, ". ").replace(/[📊|📅|⚡|✅|🧠|💾|🔴|🟡|🟢|🌊|🎉|⚠️|📄]/g, "").trim();
  if (ttsAutoRead && cleanText.length > 0 && window.speechSynthesis) speak(text);
}

function renderMemoryList(data) {
  const categories = data.categories || [];
  if (categories.length === 0) { addMessage("assistant", "No memories saved yet."); return; }
  let html = `🧠 **Saved Memories** (${data.total} total)\n\n`;
  for (const cat of categories) {
    const items = data.grouped[cat] || [];
    html += `**${cat}** (${items.length})\n`;
    for (const item of items.slice(0, 5)) html += `  • ${item.content.slice(0, 80)}${item.content.length > 80 ? "…" : ""}\n`;
...
function renderCoSResponse(action, data) {
  const el = document.createElement("div"); el.className = "message cos";
  ...
  const content = document.createElement("div"); content.className = "msg-content"; content.style.whiteSpace = "pre-wrap";

  let textContent = "";
  if (action === "morningBriefing") { content.innerHTML = formatBriefing(data); textContent = extractPlainText(data, "briefing"); }
  else if (action === "triage") { content.innerHTML = formatTriage(data); textContent = extractPlainText(data, "triage"); }
  else if (action === "followUpScan") { content.innerHTML = formatFollowUps(data); textContent = extractPlainText(data, "followups"); }
  else { content.textContent = JSON.stringify(data, null, 2); textContent = JSON.stringify(data); }
...
function extractPlainText(data, type) {
  ...
  if (type === "briefing") {
    ...
    if (data.items && data.items.length > 0) {
      text += `Triage found ${data.items.length} items. `;
      for (const item of data.items) text += `${item.severity}: ${item.title || item.type}. `;
    } else text = "All systems nominal. No urgent items.";
  } else if (type === "followups") {
    if (data.overdue?.length) text += `${data.overdue.length} overdue. `;
...
function formatTriage(data) {
  ...
  if (data.items && data.items.length > 0) { for (const item of data.items) { const icon = item.severity === "high" ? "🔴" : item.severity === "medium" ? "🟡" : "🟢"; html += `${icon} **${item.title || item.type}**\n  ${item.description || ""}\n\n`; } }
...
function formatFollowUps(data) {
  ...
  if (data.overdue?.length) { html += "**Overdue:**\n"; for (const f of data.overdue) html += `  🔴 ${f.content || f.title || "Task"}\n`; html += "\n"; }
  if (data.due_today?.length) { html += "**Due Today:**\n"; for (const f of data.due_today) html += `  🟡 ${f.content || f.title || "Task"}\n`; html += "\n"; }
  if (data.upcoming?.length) { html += "**Upcoming:**\n"; for (const f of data.upcoming.slice(0, 5)) html += `  🟢 ${f.content || f.title || "Task"}\n`; html += "\n"; }
  if (!data.overdue?.length && !data.due_today?.length && !data.upcoming?.length) html += "No follow-ups pending. You're all caught up! 🎉\n\n";
...
function addMessage(role, text) {
  ...
  const avatar = document.createElement("div"); avatar.className = "msg-avatar";
  if (role === "user") avatar.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#9b4dff" stroke-width="2"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 4-6 8-6s8 2 8 6"/></svg>`;
  else avatar.innerHTML = `<svg width="20" height="20" viewBox="0 0 56 56" fill="none"><path d="M28 4C16 4 6 14 6 26c0 8 4 14 10 18 4-6 6-10 12-10s8 4 12 10c6-4 10-10 10-18C50 14 40 4 28 4z" fill="#00e5c0" opacity="0.9"/><circle cx="28" cy="26" r="6" fill="#0a0a14"/></svg>`;
  const content = document.createElement("div"); content.className = "msg-content"; content.style.whiteSpace = "pre-wrap"; content.textContent = text;
  el.appendChild(avatar); el.appendChild(content);
  messagesEl.appendChild(el); messagesEl.scrollTop = messagesEl.scrollHeight;
  if (role === "assistant" || role === "cos") lastAssistantText = text;
}
...
async function handlePendingAction(pending) {
  ...
  if (pending.type === "chat") {
    ...
    await callWaveFunction("waveChat", { action: "chat", message: pending.text, context: pageContext || pending.pageContext });
  } else if (pending.type === "saveMemory") {
    addMessage("user", `Save this: "${pending.text.slice(0, 60)}${pending.text.length > 60 ? "…" : ""}"`);
    await callWaveFunction("waveChat", { action: "saveMemory", content: pending.text });
  }
}
...
async function fetchCredits() {
  try {
    const response = await api.runtime.sendMessage({ type: "API_CALL", payload: { function: "waveChiefOfStaff", body: { action: "morningBriefing" } } });
    if (response.ok && response.data.credits) creditInfo.textContent = `${response.data.credits.balance || "?"} credits`;
  } catch (e) { creditInfo.textContent = ""; }
}
